...
export function CheckoutScreen({
  ...
  const stockIssueSku = useMemo(() => extractStockSku(error), [error]);
  const stockIssueItemId = useMemo(
    () => cartItems.find((item) => String(item.variant?.sku || "").toLowerCase() === stockIssueSku?.toLowerCase())?.id ?? null,
    [cartItems, stockIssueSku],
  );
    ...
    <>
      <ScrollView className={`flex-1 ${dark ? "bg-slate-950" : "bg-slate-100"}`} contentContainerStyle={{ padding: 16, paddingBottom: 30 }}>
        ...
        <View className={`mt-4 rounded-3xl border p-5 ${dark ? "border-slate-700 bg-slate-900" : "border-slate-200 bg-white"}`}>
            ...
            {cartItems.map((item) => (
              <View key={item.id} className="flex-row items-start justify-between gap-3">
                <View className="flex-1">
                  ...
                  <Text className={`text-[11px] ${dark ? "text-slate-400" : "text-slate-500"}`}>
                    {item.variant?.sku ? `${item.variant.sku} · ` : ""}x {item.quantity}
                  </Text>
                  {stockIssueSku && String(item.variant?.sku || "").toLowerCase() === stockIssueSku.toLowerCase() ? (
                    <Text className="mt-1 text-[11px] font-bold text-rose-600">Out of stock now</Text>
                  ) : null}
                </View>
                <View className="items-end">
                  ...
                  <Pressable
                    ...
                    className={`mt-1 rounded-lg px-2 py-1 ${removingItemId === item.id ? "bg-slate-300" : "bg-rose-600"}`}
                  >
                    <Text className="text-[10px] font-black text-white">{removingItemId === item.id ? "..." : tr(locale, "remove")}</Text>
                  </Pressable>
                </View>
              </View>
            ))}
          ...
          {stockIssueItemId ? (
            <Pressable onPress={() => onRemoveItem(stockIssueItemId)} className="mt-2 rounded-xl bg-rose-600 py-2">
              <Text className="text-center text-xs font-black text-white">Remove unavailable item ({stockIssueSku})</Text>
            </Pressable>
          ) : null}
...
function extractStockSku(message: string): string | null {
  if (!message) return null;
  const hit = message.match(/Insufficient stock for\s+([^.\n]+)/i);
  return hit?.[1] ? String(hit[1]).trim() : null;
}
